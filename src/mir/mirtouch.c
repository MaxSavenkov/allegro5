...
static void mir_touch_input_handle_begin(int id, double timestamp,
...
static void mir_touch_input_handle_end(int id, double timestamp,
   float x, float y, bool primary, ALLEGRO_DISPLAY *disp)
{
...
static void mir_touch_input_handle_move(int id, double timestamp,
   float x, float y, bool primary, ALLEGRO_DISPLAY *disp)
{
   ALLEGRO_TOUCH_STATE* state = find_touch_state_with_id(id);
   (void)primary;
...
void _al_handle_mir_touch_event( const MirTouchEvent* event )
{
   if ( !installed )
      return;
   ...
   const int touch_count = mir_touch_event_point_count(event);
   int touch_index = 0;
   for ( ; touch_index < touch_count; ++touch_index )
   {
      MirTouchId id =  mir_touch_event_id(event, touch_index);
      MirTouchAction action = mir_touch_event_action(event, touch_index);
      const int x = mir_touch_event_axis_value(event, touch_index, mir_touch_axis_x);
      const int y = mir_touch_event_axis_value(event, touch_index, mir_touch_axis_y);

      const bool primary = ( touch_index == 0 ); // TODO: This is probably wrong

      switch( action )
      {
         case mir_touch_action_down: mir_touch_input_handle_begin( id, al_get_time(), x, y, primary, display ); break;
         case mir_touch_action_up: mir_touch_input_handle_end( id, al_get_time(), x, y, primary, display ); break;
         case mir_touch_action_change: mir_touch_input_handle_move( id, al_get_time(), x, y, primary, display ); break;
      }

      //generate_touch_input_event( type, timestamp, id, x, y, 0, 0, false, 0 );
   }
}

/* the driver vtable */
#define TOUCH_INPUT_MIR AL_ID('M','I','R','!')
